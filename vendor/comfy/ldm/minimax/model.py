...
class PackedLayout:
    ...
    def __init__(self, text_len, latent_t, latent_h, latent_w, audio_t, keyframes=None, audio_keyframes=None, refs=None, frame_count=None):
        ...
        if keyframes:
            # FL2VA condition rows. ``history`` is a temporal latent block that lives
            # immediately before the generated target; image anchors live on the target
            # timeline. This removes the old artificial first/last-only layout restriction.
            history_latent_frames = sum(
                int(kf.get("latent_frame_count", kf["latent"].shape[2]))
                for kf in keyframes if kf.get("anchor") == "history"
            )
            target_origin = float(text_len) + sum(_video_t_spans(history_latent_frames))
            history_time = float(text_len)
            target_times = _video_t_grid(latent_t, target_origin)
            for kf in keyframes:
                anchor = kf.get("anchor")
                if anchor is None:
                    pixel_index = kf.get("resolved_frame_index")
                    anchor = "first" if pixel_index == 0 else ("last" if frame_count is not None and pixel_index == frame_count - 1 else "frame")
                cond_frames = int(kf.get("latent_frame_count", kf["latent"].shape[2]))
                n_rows = cond_frames * frame_rows
                g = torch.empty(n_rows, 3, dtype=torch.float64).view(cond_frames, frame_rows, 3)
                if anchor == "history":
                    g[:, :, 0] = _video_t_grid(cond_frames, history_time)[:, None]
                    history_time += sum(_video_t_spans(cond_frames))
                elif anchor == "first":
                    g[:, :, 0] = target_times[:cond_frames, None]
                elif anchor == "last":
                    g[:, :, 0] = target_origin + sum(_video_t_spans(latent_t)) - FRAME_RESCALE
                elif anchor == "frame":
                    pixel_index = int(kf["resolved_frame_index"])
                    g[:, :, 0] = target_origin + pixel_index * FRAME_RESCALE
                else:
                    raise ValueError(f"Unknown MiniMax H3 keyframe anchor {anchor!r}")
                g[:, :, 1:] = frame[None]
                g = g.view(n_rows, 3)
                segments.append(("cond", n_rows))
                pos.append(g)
                img_pos.append(torch.arange(row, row + n_rows))
                img_update.append(torch.zeros(n_rows, dtype=torch.bool))
                row += n_rows
            cursor = target_origin

        target_audio_w = (float(w_grid[0]), float(w_grid[-1]))
        # FL2VA continuation audio conditions mirror the visual history timeline.
        # History audio starts at the pre-target origin; the boundary audio is
        # anchored to the first instant of the generated target.
        if audio_keyframes:
            if keyframes:
                history_video_frames = sum(
                    int(kf.get("latent_frame_count", kf["latent"].shape[2]))
                    for kf in keyframes if kf.get("anchor") == "history"
                )
            ...
            for akf in audio_keyframes:
                ...
                if length <= 0:
                    continue
                if anchor == "history":
                    origin = audio_history_time
                    audio_history_time += float(length)
                elif anchor == "first":
                    origin = audio_target_origin
                else:
                    raise ValueError(f"Unknown MiniMax H3 audio condition anchor {anchor!r}")
                n = length * 2
                segments.append(("cond_audio", n))
                pos.append(_audio_grid(origin, length, *target_audio_w))
