...
const APP_VERSION="2.9.1-beta.3";
...
function continuationLock(v){return v.continueEnabled?"CONTINUATION VIDEO LOCK: This clip continues directly from the previous video. Treat the incoming first frame and previous clip state as authoritative. Start from the exact existing subject identity, appearance, clothing, pose/state, props, environment layout, lighting direction, camera geography and spatial relationships. Do not redesign, replace or newly establish them. Do not begin with a hard cut, jump to a different setup, or invent a new location, wardrobe, character, prop or time-of-day change unless the user's prompt explicitly requests that change. Begin as a seamless continuation of the preceding motion and only develop the action requested by the user.":"";}
...
function enforceVideoStyle(v,result){
  const text=String(result||"").trim(),locks=[continuationLock(v),selectedStyleLock(v),preservedPerspectiveLock(v)].filter(Boolean),lock=locks.join("\n");if(!text)return text;
  const missing=locks.filter(item=>!text.includes(item));
...
function continuityAnchor(v){
  ...
  if(v.continueEnabled)return `Continue from the exact visual state inherited from the previous video; do not re-establish or redesign the subject, wardrobe, location, props, lighting, camera geography or spatial relationships.${reference} Continue only this requested action: ${finish(sanitizeStyleReferences(v.idea))}`;
...
${v.continueEnabled?`This is a CONTINUATION clip. Do not invent or replace characters, clothing, props, location, architecture, lighting, time of day or scene setup. The previous clip's final visual state is authoritative. Start directly from that state with no hard cut or new establishing setup. Add only camera/action detail needed to realize the user's requested continuation, and preserve identity, geography and object state exactly.`:`Invent the actual visible story. Every shot must contain a different, concrete action performed by the specified people or subjects. Flesh out the premise with specific props, expressions, choreography, physical cause and effect, environmental reactions and a decisive ending.`} Never write placeholders such as “establish the scene,” “the action unfolds,” “show the consequence,” “talks about,” or “resolves the idea.”
...
function directGenerationPrompt(v){
  ...
  return `Write one finished, copy-ready MiniMax Hailuo H3 video prompt. ${v.continueEnabled?"This is a continuation of an existing video, so preserve rather than reinvent the established scene.":"Expand the premise into a specific visual story."} Output only the prompt. Begin immediately with the visual description; do not add a title, heading or label such as “MiniMax Hailuo H3 Video Prompt.”

${v.continueEnabled?`${continuationLock(v)}\n\nCRITICAL CONTINUATION WRITING RULE: The first described frame must be a direct continuation of the previous video's last frame. Do not open on a new establishing shot, do not hard cut to a new angle/setup, and do not invent appearance, wardrobe, location, prop, lighting or spatial changes that the user did not request. Prefer continuous camera motion/reframing over edits. `:""}Start with a compact paragraph locking the ${v.style} look, characters, wardrobe, location and lighting. The selected visual style is authoritative: preserve its medium, rendering method, materials, palette, lighting and camera language; never replace it with a generic cinematic look or another medium. ${isTrueContinuousFlow(v)?"Write the entire clip as one uninterrupted take with no timestamps, shot numbers, beat numbers, cuts or edit points. Camera changes must be achieved only through visible continuous movement and reframing.":isMidpointContinuousFlow(v)?"Write one uninterrupted take with exactly one midpoint timing marker. The midpoint is only a story guidance marker and must not cause any cut or discontinuity.":isNoHardCutsFlow(v)?"Complete every timed beat below, but connect beats without visible hard cuts; use continuous movement, occlusion, match movement, whip movement, dissolves or other soft/hidden transitions.":"Complete every exact time range below. Each shot needs a different concrete story event, purposeful framing or camera move, visible performance and synchronized sound."} ${v.continueEnabled?"Do not invent new props, backstory, wardrobe, characters, locations or visual setup. Add only motion, camera and performance detail that directly executes the user's requested continuation.":"Invent useful actions, props, reactions, cause and effect, and a decisive ending instead of repeating the premise."} ${dialogueRule}
...
QUALITY TEST: ${v.continueEnabled?"Do not invent new scene facts. Every added detail must preserve the previous clip and only clarify the requested continuing action.":"Several concrete story events must be newly invented from the seed."} ${isTrueContinuousFlow(v)?"The result is invalid if it contains timestamps, shot/beat numbering, or any cut/edit instruction.":isMidpointContinuousFlow(v)?"The result is invalid if it contains more than the single required midpoint marker or treats that marker as a cut.":"The result is invalid if any required time range is missing, altered, generic, or left empty."}
...
function loadHistoryEntry(id){
  ...
  $("soundEnabled").checked=v.soundEnabled!==false;$("referenceEnabled").checked=Boolean(v.referenceEnabled);$("continueEnabled").checked=Boolean(v.continueEnabled);$("referenceField").hidden=!$("referenceEnabled").checked;
  $("optionalDetails").open=Boolean(v.dialogue||v.sound||v.avoid||v.referenceEnabled||v.referenceLock||v.continueEnabled);resetImagePrompt();$("promptOutput").value=item.output||"";update();$("historyDialog").close();$("idea").scrollIntoView({behavior:"smooth",block:"center"});show("Prompt and all settings restored from history.",true);
}
